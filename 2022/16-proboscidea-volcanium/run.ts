...
async function part1(testCase: TestCase = 'example.txt') {
  ...
  for await (const line of file.readLines()) {
    const { valve, rate: rawRate, tunnels: rawTunnels } = match(
      line,
  ...
  for (let [valveName, valve] of Object.entries(valveMap)) {
    ...
    while (edge = edges.pop()) {
      ...
      if (v.rate === 0) {
        seen.add(edge.valve)
        edges.push(...v.edges.map(e => ({ valve: e.valve, minutes: mins + 1})));
  ...
  function optimalPressure(opened: Set<string>, current: string, minutes: number): number {
    ...
    if (opened.has(current)) {
      ...
      return Math.max(...valve.edges.map(e => optimalPressure(opened, e.valve, minutes - e.minutes)));
    }
    ...
    return Math.max(...valve.edges.flatMap(e => [
